...
class AccountMove(models.Model):
    ...
    invoice_sequence_number_next = fields.Char(string='Next Number',
        ...
        inverse='_inverse_invoice_sequence_number_next', compute_sudo=True)
    invoice_sequence_number_next_prefix = fields.Char(string='Next Number Prefix',
        compute="_compute_invoice_sequence_number_next", compute_sudo=True)
    ...
    def write(self, vals):
        ...
        if len(vals) > 1 or (not vals.get("practical_commission_move_id", False) and not vals.get("has_reversal", False)):  # -------------------回写预提转应付时忽略校验锁定日期 --by hcw 20200504
            self._check_fiscalyear_lock_date()
    ...
    def _check_reconciliation(self):
        for line in self:
            if line.matched_debit_ids or line.matched_credit_ids:
                raise UserError(_("当前凭证已完成核销,请先取消核销后再执行操作.凭证名称: %s (%s)") % (line.move_id.name, line.move_id.id))
