...
class AccountReconciliation(models.AbstractModel):
    ...
    def get_move_lines_for_manual_reconciliation(self, account_id, partner_id=False, excluded_ids=None, search_str=False, offset=0, limit=None, target_currency_id=False):
        ...
        lines = Account_move_line.search(domain, offset=offset, limit=limit, order="date_maturity desc, partner_id, id desc")
    ...
    def process_move_lines(self, data):
        ...
        for datum in data:
            # Modified By HuChuanwei 核销时，如果核销对象存在业务伙伴，则新的核销行摘要添加合作伙伴名称-----------------
            partner_name = ''
            for ml_id in datum['mv_line_ids']:
                ml = self.env['account.move.line'].browse(ml_id)
                partner = ml.partner_id
                if partner:
                    partner_name = partner.name
                    break
            for new_ml_dict in datum['new_mv_line_dicts']:
                if partner_name:
                    new_ml_dict['name'] += ' %s' % partner_name
            # -------------------------------------------------------------------------------------------------
